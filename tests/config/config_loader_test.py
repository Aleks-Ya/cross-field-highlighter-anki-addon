...
def test_actual_values_all(config_loader: ConfigLoader, module_dir: Path):
    meta_json_config: dict[str, Any] = {"Dialog": {"Adhoc": {"Highlight": {**DefaultStopWords.config}}}}
...
def test_save_loaded_config(addon_manager: AddonManager, config_loader: ConfigLoader, module_name: str,
                            module_dir: Path):
    __write_meta_json_config({"Dialog": {"Adhoc": {"Highlight": {**DefaultStopWords.config}}}}, module_dir)
    config_origin: Optional[dict[str, Any]] = addon_manager.getConfig(module_name)
    assert config_origin == {"Dialog": {"Adhoc": {"Highlight": {**DefaultStopWords.config}}}}
    config: Config = config_loader.load_config()
    assert config.get_as_dict() == {"Dialog": {"Adhoc": {"Highlight": {**DefaultStopWords.config}}}}
    config_saved: Optional[dict[str, Any]] = addon_manager.getConfig(module_name)
    assert config_saved == {"Dialog": {"Adhoc": {"Highlight": {**DefaultStopWords.config}}}}
...
def test_write_config(config_loader: ConfigLoader, module_dir: Path, basic_note_type_name: NoteTypeName) -> None:
    ...
    assert act_config.get_as_dict() == {"Dialog": {"Adhoc": {"Highlight": {**DefaultStopWords.config}}}}
