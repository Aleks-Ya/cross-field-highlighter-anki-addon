...
class OpStatisticsFormatter:
    ...
    def format(self, op_statistics: OpStatistics):
        row1: str = f"Notes selected in Browser: {op_statistics.get_value(OpStatisticsKey.NOTES_SELECTED)}"
        row2: str = f"Notes processed: {op_statistics.get_value(OpStatisticsKey.NOTES_PROCESSED)}"
        row3: str = f"Notes modified: {op_statistics.get_value(OpStatisticsKey.NOTES_MODIFIED)}"
        return "\n".join([row1, row2, row3])
