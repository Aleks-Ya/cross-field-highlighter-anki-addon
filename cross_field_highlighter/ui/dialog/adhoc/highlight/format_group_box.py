...
class FormatGroupBox(QGroupBox, AdhocHighlightDialogModelListener):
    ...
    def model_changed(self, source: object) -> None:
        if source != self:
            ...
            if self.__model.get_current_state().selected_format:
                self.__combo_box.set_current_text(self.__model.get_current_state().selected_format.name)

    def __on_current_text_changed(self, _: str) -> None:
        highlight_format: HighlightFormat = self.__combo_box.get_current_data()
        if self.__model.get_current_state().selected_format != highlight_format:
            self.__model.get_current_state().selected_format = highlight_format
